...
public class tucows  {
	...
	static String downloadFilepath = "/home/user/Downloads/tucowsfiles";
	...
	public static void insert_into_app_tb(Connection conn, String ID, String url, boolean dl_success, boolean signed, int err_code_rsp, String exception, Float float1, String file_size_unit){
		String sql = "INSERT INTO tucows (ID, DL_Portal, DL_Success, Signed, ERR_Code_RSP, Exception, File_Size, File_Size_Unit)" + 
				"VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		try{
	...
	public static void find_win_category_links(Connection conn, WebDriver driver) throws InterruptedException
	{
		...
		List<WebElement> highlightedLinks = driver.findElements(By.xpath("//*[@id=\"titles\"]/div[*]/div[1]/a")); //*[@id="sjmp"]/div[8]/h4/a
		System.out.println("links number = " + highlightedLinks.size());
		for(WebElement el: highlightedLinks) // take the Url of each link       
		{
			...
			try{
				...
				wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("#wrap > div > div.center_content > div.left_container > div.middle_content > div > div.item_details_block > div.item_details_left > a > div")));//if link is not appeared, Malicious URL probably detected!
				driver1.findElement(By.cssSelector("#wrap > div > div.center_content > div.left_container > div.middle_content > div > div.item_details_block > div.item_details_left > a > div")).click(); //click on "download"
				...
				try{
					...
					Elements e = doc.select("#wrap > div > div.center_content > div.left_container > div.middle_content > div > div.item_details_block > div.item_details_left > div.file_size > strong");
					if (e != null)
	...
	public static void main(String[] args) {
		...
		try{ 
			...
			String hlink = "";
			List<String> pages = Arrays.asList("http://www.tucows.com/windows/audio-video/audio/audio-video-suites");
			...
			for (int i = 0; i < pages.size(); i++) {
				...
				while(driver.findElements(By.cssSelector("#titlesnavtop > a")).size() != 0)//collect links from next pages 
				{
					WebElement el = driver.findElement(By.cssSelector("#titlesnavtop > a"));
